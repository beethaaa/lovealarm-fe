            ...
            {filtered.length > 0 ? (
              <View style={styles.dropdownChipRow}>
                {filtered.map(item => (
                  ...
                  >
                    <Text
                      style={[styles.dropdownChipText, { color: chipColor }]}
                    >
                      {item}
                    </Text>
                  </TouchableOpacity>
              ...
              !showAddButton && <Text style={styles.noResult}>No results</Text>
            )}
...
const EditProfileScreen = () => {
  ...
  const [avatarUri, setAvatarUri] = useState<string | null>(
    initialUser?.avatarUrl || initialUser?.avatar || null,
  );
    ...
    const fetchInterests = async () => {
      try {
        ...
        const data = Array.isArray(raw)
          ? raw
          : raw?.interests ?? raw?.data ?? raw?.results ?? [];
        if (Array.isArray(data)) {
          const names: string[] = data.map((d: any) =>
            typeof d === 'string'
              ? d
              : d.interest ?? d.name ?? d.title ?? d.label ?? String(d),
          );
  ...
  useEffect(() => {
    const fetchTags = async () => {
      try {
        ...
        const data = Array.isArray(raw)
          ? raw
          : raw?.tags ?? raw?.data ?? raw?.results ?? [];
        if (Array.isArray(data)) {
          const names: string[] = data.map((d: any) =>
            typeof d === 'string'
              ? d
              : d.tag ?? d.name ?? d.title ?? d.label ?? String(d),
          );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          '[EditProfileScreen] Failed to refresh profile after update:',
          err,
        );
      }
